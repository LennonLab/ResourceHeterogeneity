...
mol.mod <- adonis(hmwf.bray.REL ~ design$Molecule)
mol.mod
...
points(dat1D$S.rar ~ dat1D$PCA,
       pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
legend("bottomright", legend = bquote(italic(N.S.)), bty = "n", cex = 1)
...
points(dat1D$simpsE ~ dat1D$PCA,
       pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
legend("topright", legend = bquote(italic(N.S.)), bty = "n", cex = 1)
...
points(dat2D$simpsE ~ dat2D$S.res,
       pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
legend("bottomright", legend = bquote(italic(P) == .(mod4.p)), bty = "n", cex = 1)
...
hmwf.bray.D.rda <- capscale(Bray.REL.D ~ dat1D$PCA, comm = OTUsREL.D, add = T)
hmwf.bray.R.rda <- capscale(Bray.REL.R ~ dat1R$PCA, comm = OTUsREL.R, add = T)
hmwf.bray.D.rda2012 <- capscale(Bray.REL.D2012 ~ dat2D$PCA, comm = OTUsREL.D2012, add = T)
hmwf.bray.R.rda2012 <- capscale(Bray.REL.R2012 ~ dat2R$PCA, comm = OTUsREL.R2012, add = T)

anova(hmwf.bray.D.rda2012, permutations = how(nperm=9999))
RsquareAdj(hmwf.bray.D.rda2012)
anova(hmwf.bray.R.rda2012, permutations = how(nperm=9999))
RsquareAdj(hmwf.bray.R.rda2012)
...
hmwf.bray.D.2.rda <- capscale(Bray.REL.D.2 ~ dat2D.2$PCA, 
                            comm = OTUsREL.D.2, add = T)

anova(hmwf.bray.D.2.rda, permutations = how(nperm=9999))
RsquareAdj(hmwf.bray.D.2.rda)

# DOM Diversity dbRDA
hmwf.bray.D.DOM.rda <- capscale(Bray.REL.D2012 ~ pcoa.res$points, 
                                comm = OTUsREL.D2012, add = T)
hmwf.bray.R.DOM.rda <- capscale(Bray.REL.R2012 ~ pcoa.res$points,
                                comm = OTUsREL.R2012, add = T)

anova(hmwf.bray.D.DOM.rda, permutations = how(nperm=9999))
RsquareAdj(hmwf.bray.D.DOM.rda)
anova(hmwf.bray.R.DOM.rda, permutations = how(nperm=9999))
RsquareAdj(hmwf.bray.R.DOM.rda)
...
hmwf.bray.res2 <- vegdist(resREL2, method = "bray")
pcoa.res2 <- cmdscale(hmwf.bray.res2, eig = TRUE, k = 2)
# pcoa.res2 <- pcoa.res$points[rownames(pcoa.res$points) != "Pony" &
#                              rownames(pcoa.res$points) != "Lily", ]

hmwf.bray.D.DOM.rda2 <- capscale(Bray.REL.D2012.2 ~ pcoa.res2$points, add = T)
anova(hmwf.bray.D.DOM.rda2, permutations = how(nperm=9999), model = "direct")
RsquareAdj(hmwf.bray.D.DOM.rda2)
...
hmwf.pcoa.D <- cmdscale(Bray.REL.D2012, k = 2, eig = T)
explainvar1 <- round(hmwf.pcoa.D$eig[1] / sum(hmwf.pcoa.D$eig), 3) * 100
explainvar2 <- round(hmwf.pcoa.D$eig[2] / sum(hmwf.pcoa.D$eig), 3) * 100
...
# Initiate Plot 1
plot(hmwf.pcoa.D$points[ ,1], hmwf.pcoa.D$points[ ,2], 
...
# Add Points & Labels
points(hmwf.pcoa.D$points[ ,1], hmwf.pcoa.D$points[ ,2], pch = 22, 
       cex = 2, bg = "gray", lwd = 2)

# Resource Concentration Vector
res.con <- envfit(Bray.REL.D2012, dat2D$PCA)
con.arrows <- res.con[[1]]$arrows * 0.5
arrows(0, 0, con.arrows[, 1], con.arrows[, 2], col = "red", length = 0.1, lwd = 2)
text(con.arrows[, 1] * 1.1, con.arrows[, 2] * 1, "Conc.", col = "red", cex = 1, pos = 1)

# DOM Composition Vectors
res.com <- envfit(Bray.REL.D2012, pcoa.res$points[,1:2])
...
text(com.arrows[1, 1] * 1.2, com.arrows[1, 2] * 1.2, "DOM 1", col = "red", cex = 1)
text(com.arrows[2, 1] * 1, com.arrows[2, 2] * 1.15, "DOM 2", col = "red", 
     cex = 1, pos = 1)

text(0.45, -0.035, "Pony", col = "black", cex = 0.8)
text(0.58, 0.13, "Lily", col = "black", cex = 0.8)
