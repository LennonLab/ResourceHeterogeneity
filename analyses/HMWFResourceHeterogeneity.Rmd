...
DOC2 <- data.frame("sample" = DOC$sample, "year" = DOC$year, "conc" =  DOC$conc)[order(DOC$sample, DOC$year), ]
...
TN2 <- data.frame("sample" = TN$sample, "year" = TN$year, "conc" = TN$conc)[order(TN$sample, TN$year), ]
...
TP2 <- data.frame("sample" = TP$sample, "year" = TP$year, "conc" = TP$conc)[order(TP$sample, TP$year), ]
...
OTUs <- OTUs.hmwf[, which(colSums(OTUs.hmwf) == "NA")  >= 2)]
...
ordihull(cbind(hmwf.pcoa$points[ ,1], hmwf.pcoa$points[ ,2]),
    paste(design$Lake, design$Year), kind="se", conf=0.95,
