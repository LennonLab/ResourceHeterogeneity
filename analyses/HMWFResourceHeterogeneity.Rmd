...
```{r, results="asis"}
bac.div <- cbind(alpha.div, faiths)
...
test <- t.test(div.2011$S.obs, div.2012$S.obs, paired = T)

div.D <- bac.div[bac.div$Molecule == "DNA", ]
div.R <- bac.div[bac.div$Molecule == "RNA", ]
test <- t.test(div.D$S.obs, div.R$S.obs, paired = T)
...
```{r}
par(opar)
hmwf.pcoa.b <- cmdscale(uni.frac.w, eig = TRUE, k = 3) 
