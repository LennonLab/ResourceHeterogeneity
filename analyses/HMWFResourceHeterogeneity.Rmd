...
# H1: Do resource Concentrations explain variation in bacterial communities or DOM 
...
env.chem <- as.matrix(env.chem[sort(row.names(env.chem)), ])
env.chem.L <- env.chem[nuts$sample != "Pony" & nuts$sample != "Lily", ]
env.chem2012 <- env.chem[grep("2012", row.names(env.chem)), ]
...
# Conduct dbRDA 
hmwf.bray.D.rda <- capscale(Bray.REL.D ~ env.chem, scale = T, comm = OTUsREL.D, add = T)
hmwf.bray.R.rda <- capscale(Bray.REL.R ~ env.chem, scale = T)
hmwf.uni.D.rda <- capscale(Uni.REL.R ~ env.chem, scale = T)
hmwf.uni.R.rda <- capscale(Uni.REL.R ~ env.chem, scale = T)
hmwf.res.rda <- capscale(Bray.OM ~ env.chem2012, scale = T)

# Permutation Tests
anova(hmwf.bray.D.rda, by = "axis")
anova(hmwf.bray.R.rda, by = "margin")
cca.fit <- envfit(hmwf.bacD.cca, env.chem, perm = 999)
cca.R.fit <- envfit(hmwf.bacR.cca, env.chem, perm = 999) # Only Phosphorus
cca.fit
cca.R.fit

RsquareAdj(hmwf.bacD.cca)
RsquareAdj(hmwf.bacR.cca)

# Calculate Explained Variation
cca.D.explainvar1 <- round(hmwf.bacD.cca$CCA$eig[1] / 
                         sum(c(hmwf.bacD.cca$CCA$eig, hmwf.bacD.cca$CA$eig)), 3) * 100
cca.D.explainvar2 <- round(hmwf.bacD.cca$CCA$eig[2] / 
                         sum(c(hmwf.bacD.cca$CCA$eig, hmwf.bacD.cca$CA$eig)), 3) * 100
cca.R.explainvar1 <- round(hmwf.bacR.cca$CCA$eig[1] / 
                         sum(c(hmwf.bacR.cca$CCA$eig, hmwf.bacR.cca$CA$eig)), 3) * 100
cca.R.explainvar2 <- round(hmwf.bacR.cca$CCA$eig[2] / 
                         sum(c(hmwf.bacR.cca$CCA$eig, hmwf.bacR.cca$CA$eig)), 3) * 100
```
