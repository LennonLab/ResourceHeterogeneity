...
mod2 <- lm(simpsE ~ PCA, data = dat2R[dat2R$Lake != "Pony", ])
summary(mod1);summary(mod2)

# Resource Heterogeneity and Divesity (Active)
mod3 <- lm(S.rar ~ S.res, data = dat2R[dat2R$Lake != "Pony", ])
...
# Correlation Test
cor.test(~ S.res + PCA, data = dat2R[dat2D$Lake != "Pony", ])
```
...
axis(1, lwd = 2, labels = T, at = c(0, 0.5, 1, 1.5, 2))
axis(1, lwd = 2, tck = -0.02, labels = F)
...
axis(1, lwd = 2, labels = T, las = 1, at = c(seq(520, 570, 10)))
...
arrows(0, 0, com.arrows[1, 1], com.arrows[1, 2], col = "blue", length = 0.1, lwd = 2)
arrows(0, 0, com.arrows[2, 1], com.arrows[2, 2], col = "blue", length = 0.1, lwd = 2)
text(com.arrows[1, 1] * 1.15, com.arrows[1, 2] * 1.15, "DOM 1", col = "blue", cex = 1)
text(com.arrows[2, 1] * 1.22, com.arrows[2, 2] + 0.03, "DOM 2", col = "blue", 
     cex = 1, pos = 1)
...
legend("bottomright", c("Resource Concentration Model", "Resource Composition Model"),
       lwd = 2, col = c("red", "blue", "blue"), bty = "n")
...
OTUsREL.dom2012 <- OTUsREL.2012[,which(colSums(as.matrix(OTUsREL.2012)) > 0.005)]
...
generalists <- rownames(ConRes.matrix)[rowSums(ConRes.matrix < -0.7) > 2]
...
points(seq(1.1, 10.1, 1), spec.matrix[,2], pch = 24, cex = 2,
       bg = "cornflowerblue", lwd = 2)
points(seq(0.9, 9.9, 1), spec.matrix[,4], pch = 22, cex = 2,
       bg = "wheat3", lwd = 2)
...
legend(0.175, 1, legend = c("Generalists (Total)", "Generalists (Active)",
...
rownames(generalism) <- rownames(abundancesD)
for (j in c(1:9)){
  for (i in 1:dim(generalism)[1]){
    generalism.temp <- rownames(ConRes.matrix)[rowSums(ConRes.matrix < -0.7) <= j]
    generalism.temp.2 <- abundancesD[i, colnames(abundancesD) %in% generalism.temp]
...
mtext(side = 1, "Number of Resources", cex = 1.5, line = 2.5)
mtext(side = 2, "Cumulative Proportion", cex = 1.5, line = 3)
