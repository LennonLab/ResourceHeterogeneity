...
## Table 2: Ecosystem and Microbial Processes
```{r, results="asis"}
...
div[, 1] <- div.raw$Lake[div.raw$Molecule == "DNA" & div.raw$Year == 2011]
div[, 2] <- div.raw$S.rar[div.raw$Molecule == "DNA" & div.raw$Year == 2011]
div[, 3] <- div.raw$S.rar[div.raw$Molecule == "DNA" & div.raw$Year == 2012]
div[, 4] <- div.raw$PD[div.raw$Molecule == "DNA" & div.raw$Year == 2011]
div[, 5] <- div.raw$PD[div.raw$Molecule == "DNA" & div.raw$Year == 2012]
div[, 6] <- div.raw$S.rar[div.raw$Molecule == "RNA" & div.raw$Year == 2011]
div[, 7] <- div.raw$S.rar[div.raw$Molecule == "RNA" & div.raw$Year == 2012]
div[, 8] <- div.raw$PD[div.raw$Molecule == "RNA" & div.raw$Year == 2011]
div[, 9] <- div.raw$PD[div.raw$Molecule == "RNA" & div.raw$Year == 2012]

div[, 2:9] <- round(div[, 2:9], 0)
...
OTU.tab.l <- otu_table(OTUsREL, taxa_are_rows = F)
...
## Figure 1: Bacterial Community Composition Ordination Figures (Rel Abundance)
```{r, results='hide'}
...
OTUsREL.R <- OTUsREL[design$Molecule == "DNA" , ]

OTUsREL.D.L <- OTUsREL[design$Molecule == "DNA" & 
                           design$Lake != "Pony" & design$Lake != "Lily", ]
OTUsREL.R.L <- OTUsREL[design$Molecule == "RNA" & 
                           design$Lake != "Pony" & design$Lake != "Lily", ]

# Conduct CCA 
hmwf.bacD.cca <- vegan::cca(OTUsREL.D.L ~ env.chem.L, scale = T)
hmwf.bacR.cca <- vegan::cca(OTUsREL.R.L ~ env.chem.L, scale = T)

# Permutation Tests
cca.ax <- anova(hmwf.bacD.cca, by = "axis")
cca.ev <- anova(hmwf.bacD.cca, by = "margin")
cca.fit <- envfit(hmwf.bacD.cca, env.chem.L, perm = 999)
cca.R.fit <- envfit(hmwf.bacR.cca, env.chem.L, perm = 999) # Only Phosphorus
cca.fit
cca.R.fit
...
cca.explainvar1 <- round(hmwf.bacD.cca$CCA$eig[1] / 
                         sum(c(hmwf.bacD.cca$CCA$eig, hmwf.bacD.cca$CA$eig)), 3) * 100
cca.explainvar2 <- round(hmwf.bacD.cca$CCA$eig[2] / 
                         sum(c(hmwf.bacD.cca$CCA$eig, hmwf.bacD.cca$CA$eig)), 3) * 100
```
...
```{r}
env.chem2 <- env.chem[rep(1:20, each = 2), ]
...
nitro <- lm(rowSums(active.NPA)/rowSums(totalPA) ~ log10(nuts$TN[c(order(nuts$sample))]))
carb <- lm(rowSums(active.NPA)/rowSums(totalPA) ~ log10(nuts$DOC[c(order(nuts$sample))]))
mod1 <- summary(phos)
