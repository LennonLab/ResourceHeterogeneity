...
sum.eig.res <- sum(explainvar1.res, explainvar2.res)

# DOM Scores
dom.scores <- add.spec.scores(pcoa.res,resREL,method="cor.scores",multi=1,Rscale=F,scaling="1")
...
# Statistical Description of BCC
```{r}
# Observed Richness
...
div.mod2 <- Anova(lm(shan ~ Lake + Molecule, data = alpha.div[alpha.div$Lake != 
...
```{r}
# Nutrients and DOM
cor(res.div, log10(nuts[nuts$Year == "2012", 3:5]))
cor.test(res.div$S.res, log10(nuts$TN[nuts$Year == "2012"]))
cor(res.div[rownames(res.div) != "Pony", ], 
    log10(nuts[nuts$Year == "2012" & nuts$Site != "Pony", 3:5]))
cor.test(res.div$S.res[rownames(res.div) != "Pony"],  #Dropped Pony
         log10(nuts$DOC[nuts$Year == "2012" & nuts$Site != "Pony"]))

# Nutrients and BCC
cor(alpha.div[alpha.div$Molecule == "DNA", 5:7], log10(nuts[, 3:5]))
cor(alpha.div[alpha.div$Molecule == "RNA", 5:7], log10(nuts[, 3:5]))

# DOM and BCC
cor(alpha.div[alpha.div$Molecule == "DNA" & alpha.div$Year == "2012", 5:7], res.div)
cor(alpha.div[alpha.div$Molecule == "RNA" & alpha.div$Year == "2012", 5:7], res.div)
cor.test(alpha.div$simpsE[alpha.div$Molecule == "RNA" & alpha.div$Year == "2012"], res.div$S.res)
```

## Only Significant Correlation is S.Even and DOM.Rich
```{r, results = "hide"}


# Scale and Center Nutrients
nuts2012 <- nuts[nuts$Year == "2012", ]
nuts2012.s <- nuts2012
nuts2012.s$DOC <- as.numeric(scale(log10(nuts2012.s$DOC)))
nuts2012.s$TN <- as.numeric(scale(log10(nuts2012.s$TN)))
nuts2012.s$TP <- as.numeric(scale(log10(nuts2012.s$TP)))

# Active Community in 2012
alpha.div2012A <- alpha.div[alpha.div$Molecule == "RNA" & 
                              alpha.div$Year == "2012", ]

# Models
mod1 <- lm(S.rar ~ TP, 
           data = data.frame(alpha.div2012A[alpha.div2012A$Lake != "Pony", ],
                        nuts2012.s[nuts2012.s$Site != "Pony", ]))
pred.frame1 <- data.frame(TP = seq(-1.8, 2.5, 0.1))

mod2 <- lm(simpsE ~ TP, 
           data = data.frame(alpha.div2012A[alpha.div2012A$Lake != "Pony", ],
                        nuts2012.s[nuts2012.s$Site != "Pony", ]))
pred.frame2 <- data.frame(TP = seq(-1.8, 2.5, 0.1))

mod3 <- lm(S.rar ~ S.res, 
           data = data.frame(alpha.div2012A[alpha.div2012A$Lake != "Pony", ], 
                        res.div[rownames(res.div) != "Pony", ]))
pred.frame3 <- data.frame(S.res = seq(528,570,2))

mod4 <- lm(simpsE ~ S.res, 
           data = data.frame(alpha.div2012A[alpha.div2012A$Lake != "Pony", ], 
                        res.div[rownames(res.div) != "Pony", ]))
pred.frame4 <- data.frame(S.res = seq(528,570,2))
...
# Nuts vs Sbac
plot(nuts2012.s$TP, alpha.div2012A$S.rar, 
     xlab = "", ylab = "", axes = F,
     xlim = c(-2, 2.6), ylim = c(0, 2200), las = 1,
...
points(nuts2012.s$TP, alpha.div2012A$S.rar, 
       pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
points(nuts2012.s$TN, alpha.div2012A$S.rar, 
     pch = 21, col = "black", bg = "gray", cex = 2, lwd = 2)
mtext("Nutrients", side = 1, line = 3, cex = 1.5)
mtext("Species Richness", side = 2, line = 3.5, cex = 1.5)
legend("topleft", legend = c("TP", "TN"), bty = "n", pch = c(22, 21),
       col = "black", pt.bg = "gray", pt.lwd = 2, pt.cex = 1.5, bg = "white")
axis(1, lwd = 2, labels = c("low", "medium", "high"), at = c(-2, 0, 2))
...
# Nuts vs Ebac
plot(nuts2012.s$TP, alpha.div2012A$simpsE, 
     xlab = "", ylab = "", axes = F,
     xlim = c(-2, 2.6), ylim = c(0, 0.06), las = 1,
     pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
add.hull(model = mod2, pred.frame = pred.frame2)
matlines(pred.frame2, predict(mod2, interval = "c", newdata=pred.frame2),
         lty=c(2,3,3), lwd=c(4,2,2), col="black")
points(nuts2012.s$TP, alpha.div2012A$simpsE,
       pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
points(nuts2012.s$TN, alpha.div2012A$simpsE, 
     pch = 21, col = "black", bg = "gray", cex = 2, lwd = 2)
mtext("Nutrients", side = 1, line = 3, cex = 1.5)
mtext("Species Evenness", side = 2, line = 3.5, cex = 1.5)
legend("topright", legend = c("TP", "TN"), bty = "n", pch = c(22, 21),
       col = "black", pt.bg = "gray", pt.lwd = 2, pt.cex = 1.5, bg = "white")
axis(1, lwd = 2, labels = c("low", "medium", "high"), at = c(-2, 0, 2))
axis(2, lwd = 2, labels = T, las = 1)
axis(3, lwd = 2, tck = -0.02, labels = F)
...
# Sdom vs Sbac
plot(res.div$S.res, alpha.div2012A$S.rar, 
     xlab = "", ylab = "", type = "n",
     xlim = c(525, 572), ylim = c(0, 2200), las = 1,
     pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
add.hull(model = mod3, pred.frame = pred.frame3)
matlines(pred.frame3, predict(mod3, interval = "c", newdata=pred.frame3),
         lty=c(2,3,3), lwd=c(4,2,2), col="black")
points(res.div$S.res, alpha.div2012A$S.rar,
       pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
...
# Sdom vs Ebac
plot(res.div$S.res, alpha.div2012A$simpsE, 
     xlab = "", ylab = "", type = "n",
     xlim = c(525, 572), ylim = c(0, 0.06), las = 1,
     pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
add.hull(model = mod4, pred.frame = pred.frame4)
matlines(pred.frame4, predict(mod4, interval = "c", newdata=pred.frame4),
         lty=c(2,3,3), lwd=c(4,2,2), col="black")
points(res.div$S.res, alpha.div2012A$simpsE,
       pch = 22, col = "black", bg = "gray", cex = 2, lwd = 2)
mtext("DOM Richness", side = 1, line = 3, cex = 1.5)
mtext("Species Evenness", side = 2, line = 3.5, cex = 1.5)
axis(1, lwd = 2, labels = F)
...
## Supp Figure 1: Organic Matter Ordination Figure
```{r, results='hide'}
...
png(filename="../figures/SuppFigure1.png",
