...
div$Lake <- div.raw$Lake[div.raw$Molecule == "DNA" & div.raw$Year == 2011]
div[, 2] <- div.raw$R.rar[div.raw$Molecule == "DNA" & div.raw$Year == 2011]
div[, 3] <- div.raw$R.rar[div.raw$Molecule == "DNA" & div.raw$Year == 2012]
div[, 4] <- div.raw$PD[div.raw$Molecule == "DNA" & div.raw$Year == 2011]
div[, 5] <- div.raw$PD[div.raw$Molecule == "DNA" & div.raw$Year == 2012]
div[, 6] <- div.raw$R.rar[div.raw$Molecule == "RNA" & div.raw$Year == 2011]
div[, 7] <- div.raw$R.rar[div.raw$Molecule == "RNA" & div.raw$Year == 2012]
...
addtorow$command <- c("& \\multicolumn{4}{c}{Total} & 
                      ...
                      2011 & 2012 & 2011 & 2012 \\\\\n")
div.tab <- xtable(div)
align(div.tab) <- c("c ","r ", "r ", "r ", "r ","r ", "r ", "r ", "r ")
...
## Paired T-Tests
```{r}
div.2011 <- bac.div[bac.div$Year == "2011", ]
div.2012 <- bac.div[bac.div$Year == "2012", ]
t.test(div.2011$S.rar, div.2012$S.rar, paired = T)

div.D <- bac.div[bac.div$Molecule == "DNA", ]
div.R <- bac.div[bac.div$Molecule == "RNA", ]
t.test(div.D$S.rar, div.R$S.rar, paired = T)

mod1 <- lm(bac.div$S.rar ~ as.factor(bac.div$Lake) + 
             as.factor(bac.div$Year) + as.factor(bac.div$Molecule))
Anova(mod1)
...
## Figure 1: Bacterial Community Composition Ordination Figures
```{r, results='hide'}
...
layout(matrix(c(1, 1, 2, 2,
                ...
                5, 5, 6, 6,
                5, 5, 6, 6,
                7, 7, 7, 7), ncol = 4, byrow = T))
layout.show(7)
par(mar = c(1, 1, 1, 1) + 0.1, oma = c(1, 1, 1, 1))
...
pcoa.plots <- list(pcoa.pa, pcoa.rel, pcoa.log, pcoa.ufu, pcoa.ufw, pcoa.ufwl)
...
for (i in (1:length(pcoa.plots))){
  ...
  # Add Points & Labels
  points(pcoa.plots[[i]]$points[ ,1], pcoa.plots[[i]]$points[ ,2],
...
## PERMANOVA
```{r}
per.bray <- adonis(OTUsREL ~ design$Lake + design$Molecule, method = "bray")
per.uni <- adonis(uni.frac.w ~ design$Lake + design$Molecule)
per.uni.u <- adonis(uni.frac.u ~ design$Lake + design$Molecule)
per.bray.l <- adonis(OTUsREL.log ~ design$Lake + design$Molecule, method = "bray")
per.bray.p <- adonis(OTUsPA ~ design$Lake + design$Molecule, method = "bray")
...
OTUsREL.D <- OTUsREL.log[design$Molecule == "DNA" , ]
OTUsREL.R <- OTUsREL[design$Molecule == "DNA" , ]

OTUsREL.D.L <- OTUsREL.log[design$Molecule == "DNA" & 
                           design$Lake != "Pony" & design$Lake != "Lily", ]
OTUsREL.R.L <- OTUsREL.log[design$Molecule == "RNA" & 
                           design$Lake != "Pony" & design$Lake != "Lily", ]
